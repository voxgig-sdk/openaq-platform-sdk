...
func TestMeasurementEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OPENAQPLATFORM_TEST_MEASUREMENT_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func measurementBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPENAQPLATFORM_TEST_MEASUREMENT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPENAQPLATFORM_TEST_MEASUREMENT_ENTID"])
