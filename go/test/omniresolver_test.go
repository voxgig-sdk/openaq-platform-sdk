...
import (
	...
	omni "github.com/voxgig-sdk/openaq-platform-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenaqPlatformError); is {
				out := map[string]any{
					"name":    "OpenaqPlatformError",
...
func omniCtx(arg any, client *sdk.OpenaqPlatformSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenaqPlatformSDK); is {
			client = live
