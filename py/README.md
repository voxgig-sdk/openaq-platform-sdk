# OpenaqPlatform Python SDK



The Python SDK for the OpenaqPlatform API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Location()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/openaq-platform-sdk/releases)) or
...
```python
client = OpenaqPlatformSDK.test(testopts, sdkopts)
```
...
```
py/
├── openaqplatform_sdk.py         -- Main SDK module
...
The main module (`openaqplatform_sdk`) exports the SDK class.
