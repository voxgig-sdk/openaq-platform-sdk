...
The Python SDK for the OpenaqPlatform API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Location(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.OpenaqPlatform(None).load(
    {"id": "test01"}, None
)
