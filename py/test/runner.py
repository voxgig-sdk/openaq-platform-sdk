# OpenaqPlatform SDK test runner
...
class OpenaqPlatformTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenaqPlatformTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenaqPlatformTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenaqPlatformTestRunner.getenv("OPENAQ_PLATFORM_TEST_LIVE")
        override = OpenaqPlatformTestRunner.getenv("OPENAQ_PLATFORM_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenaqPlatformTestRunner.getenv(key)
        ...
        explain = OpenaqPlatformTestRunner.getenv("OPENAQ_PLATFORM_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if OpenaqPlatformTestRunner._test_control is not None:
            return OpenaqPlatformTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenaqPlatformTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenaqPlatformTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenaqPlatformTestRunner.load_test_control()
...
def load_env_local():
    OpenaqPlatformTestRunner.load_env_local()


def env_override(m):
    return OpenaqPlatformTestRunner.env_override(m)


def entity_data(v):
    return OpenaqPlatformTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenaqPlatformTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenaqPlatformTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenaqPlatformTestRunner.load_test_control()


def live_client_options():
    return OpenaqPlatformTestRunner.live_client_options()


def live_delay_ms():
    return OpenaqPlatformTestRunner.live_delay_ms()
