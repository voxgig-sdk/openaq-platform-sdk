...
//    only - a typed OpenaqPlatformContext there would make every ctx
...
final class OpenaqPlatformOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OpenaqPlatformContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new OpenaqPlatformSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new OpenaqPlatformResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new OpenaqPlatformError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new OpenaqPlatformResponse($clean['response']);
    ...
    public static function err_from_map($m): ?OpenaqPlatformError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OpenaqPlatformError(is_string($code) ? $code : '', $msg);
    }
