...
// OpenaqPlatform SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OpenaqPlatformTestFeature extends OpenaqPlatformBaseFeature
{
    ...
    public function init(OpenaqPlatformContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OpenaqPlatformContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OpenaqPlatformParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OpenaqPlatformContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OpenaqPlatformContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = OpenaqPlatformParam::call($ctx, $k);
