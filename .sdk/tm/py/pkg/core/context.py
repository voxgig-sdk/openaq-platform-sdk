...
from projectname_sdk.core.control import OpenaqPlatformControl
...
class OpenaqPlatformContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = OpenaqPlatformControl()
        ...
        if isinstance(sp, OpenaqPlatformSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, OpenaqPlatformResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, OpenaqPlatformResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return OpenaqPlatformOperation({})
    ...
    def make_error(self, code, msg):
        return OpenaqPlatformError(code, msg, self)
