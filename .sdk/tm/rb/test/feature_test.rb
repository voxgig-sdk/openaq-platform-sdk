...
module OpenaqPlatformFeatureHarness
  ...
  class Harness
    ...
    def initialize(features, server: nil, base: "http://api.test", headers: {})
      ...
      @utility.fetcher = server || OpenaqPlatformFeatureHarness.default_server
      ...
      features.each do |fspec|
        name = fspec["name"]
        next unless OpenaqPlatformFeatureHarness.has_feature?(name)
        f = OpenaqPlatformFeatures.make_feature(name)
    ...
    def op(opname: "load", entity: "widget", method: nil, path: nil, query: nil,
      ...
      method ||= OpenaqPlatformFeatureHarness.default_method(opname)
      ...
      begin
        fire(ctx, "PrePoint")
        raise ctx.out["point"] if ctx.out["point"].is_a?(OpenaqPlatformError)
        ...
        ctx.response = fetched.is_a?(Hash) ? OpenaqPlatformResponse.new(fetched) : nil
    ...
    def populate_result(ctx, fetched, fetch_err)
      result = OpenaqPlatformResult.new({})
      ctx.result = result
...
class FeatureTest < Minitest::Test
  H = OpenaqPlatformFeatureHarness
  ...
  def test_retry_retries_a_transport_error_tuple
    ...
    server, calls = H.recording_server { |n, _fd|
      n < 3 ? [nil, OpenaqPlatformError.new("boom", "boom")] : [H.make_response(200, { "ok" => true }), nil]
    }
  ...
  def test_retry_exhausted_transport_error_surfaces
    skip_unless_feature("retry")
    clock = H::Clock.new
    server, calls = H.recording_server { |_n, _fd| [nil, OpenaqPlatformError.new("boom", "boom")] }
    h = harness([fspec("retry",
      "retries" => 2, "minDelay" => 1, "jitter" => false, "sleep" => clock.sleeper)],
      server: server)
    res = h.op
