...
module OpenaqPlatformUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenaqPlatformResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenaqPlatformResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenaqPlatformResponse.new(fetched)
    else
