...
// OpenaqPlatform SDK retry feature
...
class OpenaqPlatformRetryFeature extends OpenaqPlatformBaseFeature
{
    ...
    public function init(OpenaqPlatformContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenaqPlatformContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenaqPlatformContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenaqPlatformContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
