...
// OpenaqPlatform SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OpenaqPlatformOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OpenaqPlatformOmni::structfix($now, Struct::undef()))
                        !== json_encode(OpenaqPlatformOmni::structfix($val, Struct::undef()))) {
