...
class MeasurementEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = OpenaqPlatformSDK::test($seed, null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENAQ_PLATFORM_TEST_MEASUREMENT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function measurement_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("OPENAQ_PLATFORM_TEST_MEASUREMENT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["OPENAQ_PLATFORM_TEST_MEASUREMENT_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["OPENAQ_PLATFORM_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new OpenaqPlatformSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["OPENAQ_PLATFORM_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
