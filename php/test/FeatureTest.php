...
// OpenaqPlatform SDK feature test
//
...
class FtCtrl extends OpenaqPlatformControl
{
...
class FtClient
{
    ...
    private OpenaqPlatformUtility $utility;

    public function __construct(OpenaqPlatformUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenaqPlatformUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenaqPlatformContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenaqPlatformContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenaqPlatformContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenaqPlatformConfig::make_feature($name);
    ...
    private static function buildUrl(OpenaqPlatformSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenaqPlatformError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenaqPlatformResponse($fetched);
        }
    ...
    private function fail(OpenaqPlatformContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenaqPlatformContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
