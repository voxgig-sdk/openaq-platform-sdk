...
// OpenaqPlatform SDK pipeline test
//
...
{
    private static function utility(): OpenaqPlatformUtility
    {
        return new OpenaqPlatformUtility();
    }
    ...
    private static function ctx(array $over = []): OpenaqPlatformContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenaqPlatformContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenaqPlatformOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenaqPlatformError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenaqPlatformOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenaqPlatformMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenaqPlatformOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenaqPlatformMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenaqPlatformOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenaqPlatformMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenaqPlatformSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenaqPlatformMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenaqPlatformResponse([]),
            'result' => new OpenaqPlatformResult([]),
        ]);
        [, $err] = OpenaqPlatformMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenaqPlatformSpec([]),
            'response' => null,
            'result' => new OpenaqPlatformResult([]),
        ]);
        [, $err] = OpenaqPlatformMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenaqPlatformSpec(['step' => 's']),
            'response' => new OpenaqPlatformResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenaqPlatformResult([]),
        ]);
        [, $err] = OpenaqPlatformMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenaqPlatformSpec(['step' => 's']),
            'response' => new OpenaqPlatformResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenaqPlatformResult([]),
        ]);
        [, $err] = OpenaqPlatformMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenaqPlatformSpec(['step' => 's']),
            'response' => new OpenaqPlatformResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenaqPlatformResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenaqPlatformMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenaqPlatformResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenaqPlatformSpec([]),
            'response' => new OpenaqPlatformResponse([]),
            'result' => new OpenaqPlatformResult([]),
        ...
        [$got, $err] = OpenaqPlatformMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenaqPlatformResult([])]);
        [, $err] = OpenaqPlatformMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenaqPlatformSpec([]), 'result' => null]);
        [, $err] = OpenaqPlatformMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenaqPlatformSpec(['step' => 's']),
            'result' => new OpenaqPlatformResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenaqPlatformOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenaqPlatformMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenaqPlatformSpec(['step' => 's']),
            'result' => new OpenaqPlatformResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenaqPlatformOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenaqPlatformMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenaqPlatformResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenaqPlatformSpec([]),
            'result' => new OpenaqPlatformResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenaqPlatformMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenaqPlatformMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenaqPlatformError('boom', 'boom');
        $utility->fetcher = function (OpenaqPlatformContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenaqPlatformSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenaqPlatformMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenaqPlatformContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenaqPlatformMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenaqPlatformResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenaqPlatformMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenaqPlatformContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenaqPlatformSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenaqPlatformMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenaqPlatformResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenaqPlatformSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenaqPlatformMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenaqPlatformMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenaqPlatformMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenaqPlatformResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenaqPlatformDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenaqPlatformResult(['ok' => false])]);
        $this->expectException(OpenaqPlatformError::class);
        OpenaqPlatformDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenaqPlatformResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenaqPlatformMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenaqPlatformMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenaqPlatformBaseFeature();
        $b = new OpenaqPlatformBaseFeature();
        OpenaqPlatformFeatureAdd::call($ctx, $a);
        OpenaqPlatformFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenaqPlatformBaseFeature
    {
        $f = new OpenaqPlatformBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenaqPlatformFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenaqPlatformContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenaqPlatformSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenaqPlatformPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OpenaqPlatformPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenaqPlatformResponse(['headers' => null]),
            'result' => new OpenaqPlatformResult([]),
        ]);
        OpenaqPlatformResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenaqPlatformResultBody::call($ctx);
