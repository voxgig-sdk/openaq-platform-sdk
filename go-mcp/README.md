...
[MCP](https://modelcontextprotocol.io) server exposing the OpenaqPlatform SDK as
two agent tools — `openaq-platform_list` and `openaq-platform_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openaq-platform-mcp)
make build

# 2. Provide credentials via the environment
export OPENAQ_PLATFORM_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user openaq-platform \
  -- /absolute/path/to/openaq-platform-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./openaq-platform-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// openaq-platform_list: first page of records
{ "entity": "location" }
{ "entity": "location", "query": { } }

// openaq-platform_load: one record by id
{ "entity": "location", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openaq-platform-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user openaq-platform \
     -- "$PWD"/dist/*/openaq-platform-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `openaq-platform_list` and `openaq-platform_load` tools now appear
   in new sessions. Ask the agent to *"list location using openaq-platform"*
   and it calls `openaq-platform_list` with `{"entity":"location"}`.
...
```sh
export OPENAQ_PLATFORM_APIKEY=sk_live_xxx            # API key
export OPENAQ_PLATFORM_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `openaq-platform_list` tool
...
### Call the `openaq-platform_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `openaq-platform_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `openaq-platform_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPENAQ_PLATFORM_APIKEY` | API key sent with every request. |
| `OPENAQ_PLATFORM_BASE` | Optional override of the API base URL. |
...
```sh
./openaq-platform-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"openaq-platform_load","arguments":{"entity":"location","query":{"id":1}}}}'
```
