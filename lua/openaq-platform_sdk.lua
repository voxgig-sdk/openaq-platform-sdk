...
local OpenaqPlatformSDK = {}
OpenaqPlatformSDK.__index = OpenaqPlatformSDK
...
OpenaqPlatformSDK._make_feature = _make_feature


function OpenaqPlatformSDK.new(options)
  local self = setmetatable({}, OpenaqPlatformSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OpenaqPlatformSDK:options_map()
...
function OpenaqPlatformSDK:get_utility()
  return Utility.copy(self._utility)
end


function OpenaqPlatformSDK:get_root_ctx()
  return self._rootctx
end


function OpenaqPlatformSDK:prepare(fetchargs)
...
function OpenaqPlatformSDK:direct(fetchargs)
...
-- by passing it.
function OpenaqPlatformSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OpenaqPlatformSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OpenaqPlatformSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function OpenaqPlatformSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OpenaqPlatformSDK.new(sdkopts)
  sdk.mode = "test"
