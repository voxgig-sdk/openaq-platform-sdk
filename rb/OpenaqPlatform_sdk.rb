...
require_relative 'OpenaqPlatform_types'


class OpenaqPlatformSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenaqPlatformHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenaqPlatformHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenaqPlatformFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenaqPlatformUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenaqPlatformHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenaqPlatformHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenaqPlatformHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OpenaqPlatformError.new(
        "#{op}_allow",
        "OpenaqPlatformSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue OpenaqPlatformError => err
    ...
    if fetched.is_a?(Hash)
      status = OpenaqPlatformHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenaqPlatformError.new(
        "graphql_error", "OpenaqPlatformSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpenaqPlatformSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
