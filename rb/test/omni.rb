...
module OpenaqPlatformOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if OpenaqPlatformOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = OpenaqPlatformOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| OpenaqPlatformOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || OpenaqPlatformOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = OpenaqPlatformSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = OpenaqPlatformError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { OpenaqPlatformOmni.errify(err) }
