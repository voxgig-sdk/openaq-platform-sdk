...
boru-driven command-line client **and** interactive REPL for the OpenaqPlatform
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openaq-platform-cli)
make build

# 2. See usage (words, entities, env vars)
./openaq-platform-cli --help
...
./openaq-platform-cli list measurement

# 5. Override the API base URL for a single call
OPENAQ_PLATFORM_BASE=https://api.example.com ./openaq-platform-cli list location

# 6. No arguments -> interactive REPL
./openaq-platform-cli
openaq-platform> list location
openaq-platform> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openaq-platform-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/openaq-platform-cli list location
   ```
...
Run with no arguments to open a REPL (prompt `openaq-platform>`). Each line is
evaluated as its own boru expression:

```text
$ ./openaq-platform-cli
openaq-platform> list location
openaq-platform> /help
openaq-platform> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `location`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/openaq-platform-cli`. |
...
(`./openaq-platform-cli <expr>`) and interactively (the REPL), and expressions compose the
