// Command OpenaqPlatform-cli is a CLI + REPL for the OpenaqPlatform SDK,
...
const prompt = "openaq-platform"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "location measurement"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OPENAQ_PLATFORM_APIKEY (api key), OPENAQ_PLATFORM_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OPENAQ_PLATFORM_APIKEY carries the API key and
	// OPENAQ_PLATFORM_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPENAQ_PLATFORM_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPENAQ_PLATFORM_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpenaqPlatformSDK(opts)
