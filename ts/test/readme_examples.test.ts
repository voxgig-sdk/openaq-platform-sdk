...
//     (\`new OpenaqPlatformSDK(...)\` / \`OpenaqPlatformSDK.test(...)\` become
//     \`OpenaqPlatformSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OpenaqPlatformSDK'
...
const TEST_SEED = {"entity":{"location":{"test01":{"id":"test01"}},"measurement":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenaqPlatformSDK(...)\` and any
// \`OpenaqPlatformSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenaqPlatformSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenaqPlatformSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
